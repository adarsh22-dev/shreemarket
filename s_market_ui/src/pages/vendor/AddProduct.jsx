...
import { Image as ImageIcon, Plus, ArrowLeft, ChevronRight, FileText } from 'lucide-react';
import VendorLayout from '../../components/vendor/VendorLayout';
import './AddProduct.css';
import { useNavigate, useParams } from 'react-router-dom';
import { addProduct, getProduct, updateProduct, BACKEND_URL } from '../../api/api';
...
const AddProduct = () => {
    ...
    const handleAddVariation = () => {
        setVariations([...variations, { name: '', sku: '', price: '', stock: '', useMainPricing: true }]);
    };
    ...
    const handleSaveProduct = async () => {
        ...
        try {
            const productData = {
                ...
                attributes: attributes.filter(a => a.name && a.value).map(a => ({ name: a.name, value: a.value })),
                tags: tags.map(t => ({ name: t })),
                variations: variations.filter(v => v.name).map(v => ({
                    name: v.name,
                    sku: v.sku,
                ...
                linkedProducts: [
                    ...upsells.map(u => ({ linkedType: 'UPSELL', linkedProductName: u })),
                    ...crossSells.map(c => ({ linkedType: 'CROSS_SELL', linkedProductName: c }))
                ],
            ...
            const loadingToast = toast.loading(isEditMode ? 'Updating product...' : 'Saving product...');
        ...
        <VendorLayout>
            <div className="add-product-container">
                ...
                <div className="add-product-top-nav">
                    ...
                    <button className="btn-back" onClick={handleCancel} title="Back to Products">
                        <ArrowLeft size={20} />
                    </button>
                ...
                <div className="add-product-header">
                    <div>
                        ...
                        <p>{isEditMode ? 'Update your product details here.' : 'List a new item in your marketplace store.'}</p>
                    </div>
                ...
                <div className="form-section">
                    ...
                    <div className="form-group">
                        ...
                        <input type="text" className="form-control" placeholder="e.g. Minimalist Oak Coffee Table" value={productName} onChange={(e) => setProductName(e.target.value)} />
                    </div>
                    ...
                    <div className="form-group" style={{ marginBottom: 0 }}>
                        ...
                        <textarea className="form-control" placeholder="Provide a detailed description of the product features and benefits..." value={description} onChange={(e) => setDescription(e.target.value)}></textarea>
                    </div>
                </div>
                ...
                <div className="form-section">
                    ...
                    <div className="pricing-grid">
                        ...
                        <div className="form-group" style={{ marginBottom: 0 }}>
                            <label>SKU</label>
                            <input type="text" className="form-control" placeholder="EHOME-001" value={sku} onChange={(e) => setSku(e.target.value)} />
                        </div>
                        <div className="form-group" style={{ marginBottom: 0 }}>
                    ...
                    {priceError && (
                        <div style={{ color: '#FF5722', fontSize: '0.85rem', marginTop: '0.75rem' }}>
                            {priceError}
                        </div>
                    )}
                    ...
                    {supportsWholesale && (
                        <div className="pricing-grid" style={{ marginTop: '1rem', paddingTop: '1.5rem', borderTop: '1px solid #000' }}>
                            <div className="form-group" style={{ marginBottom: 0 }}>
                ...
                <div className="form-section">
                    ...
                    >
                        ...
                        <button className="btn-file" type="button" onClick={(e) => { e.stopPropagation(); fileInputRef.current.click(); }}>Browse files</button>
                        <input
                            type="file"
                            multiple
                            accept="image/*,video/*"
                    ...
                    <div className="image-preview-row" style={{ flexWrap: 'wrap' }}>
                        {mediaFiles.map((media, index) => (
                            ...
                            >
                                {media.type === 'video' ? (
                                    ...
                                    <img src={media.url} alt={`Preview ${index}`} className="image-thumbnail" draggable="false" />
                                )}
                        ...
                        <button className="add-thumbnail-btn" type="button" onClick={() => fileInputRef.current.click()}>
                            <Plus size={24} />
                        </button>
                    </div>
                </div>
                ...
                <div className="form-section">
                    ...
                    <div className="attributes-grid" style={{ display: 'block' }}>
                        {attributes.map((attr, index) => (
                            <div key={index} className="form-row" style={{ marginBottom: '1rem', alignItems: 'flex-start' }}>
                                ...
                                <div style={{ paddingTop: index === 0 ? '24px' : '0' }}>
                                    <button
                                        ...
                                        style={{ padding: '0.75rem', borderColor: '#ffdddd', color: '#FF5722', backgroundColor: '#FFF0EB' }}
                        ...
                        <button
                            type="button"
                            className="btn-secondary"
                            style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '1rem', borderStyle: 'dashed' }}
                            onClick={handleAddAttribute}
                        >
                            <Plus size={16} /> Add Attribute
                        </button>

                        <div className="form-row" style={{ marginTop: '1.5rem', alignItems: 'flex-start' }}>
                            <div className="form-group form-col" style={{ flex: 2, marginBottom: 0 }}>
                                <label>Tags</label>
                                <input
                                    type="text"
                                    className="form-control"
                            ...
                            <div className="form-group form-col" style={{ flex: 1, marginBottom: 0 }}>
                ...
                <div className="form-section">
                    ...
                    <p style={{ color: '#000', fontSize: '0.9rem', marginBottom: '1.5rem', marginTop: '-1rem' }}>
                        Add variations of this product (e.g. different colors, sizes) if applicable. Leave empty for a simple product.
                    </p>

                    <div className="variations-grid" style={{ display: 'block' }}>
                        {variations.map((variation, index) => (
                            <div key={index} className="form-row" style={{ marginBottom: '1.5rem', padding: '1rem', backgroundColor: '#fff', borderRadius: '8px', border: '1px solid #000' }}>
                                    ...
                                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                                        <div className="checkbox-group" style={{ margin: 0 }}>
                                            <input
                                                ...
                                                onChange={(e) => handleVariationChange(index, 'useMainPricing', e.target.checked)}
                                            />
                                            <label htmlFor={`useMainPricing-${index}`}>Use main product pricing</label>
                                        </div>
                                        <button
                                            type="button"
                                            className="btn-secondary"
                                            style={{ padding: '0.5rem 0.75rem', color: '#FF5722', backgroundColor: '#FFF0EB', fontSize: '0.85rem' }}
                                    ...
                                    <div className="form-row">
                                        <div className="form-group form-col" style={{ marginBottom: 0 }}>
                                            ...
                                            <input
                                                type="number"
                                                className="form-control"
                                                placeholder={variation.useMainPricing !== false ? "Using Main Price" : "0.00"}
                                                value={variation.useMainPricing !== false ? regularPrice : variation.price}
                                                onChange={(e) => {
                    ...
                    <button
                        type="button"
                        className="btn-secondary"
                        style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '0.5rem', borderStyle: 'dashed' }}
                    ...
                    <div className="form-row">
                        <div className="form-group form-col">
                            ...
                            <p style={{ fontSize: '0.8rem', color: '#000', marginTop: '-0.25rem', marginBottom: '0.5rem', minHeight: '3.5rem' }}>
                                Products which you recommend instead of the currently viewed product, for example, products that are more profitable or better quality.
                            </p>
                            <input
                                type="text"
                                className="form-control"
                            ...
                            {upsells.length > 0 && (
                                <div className="product-tags-container">
                                    {upsells.map((item, index) => (
                                        <span key={index} className="product-tag">
                                            {item}
                                            <button type="button" onClick={() => handleRemoveUpsell(item)} className="product-tag-remove">&times;</button>
                                        </span>
                                    ))}
                                </div>
                            )}
                        ...
                        <div className="form-group form-col">
                            ...
                            <p style={{ fontSize: '0.8rem', color: '#000', marginTop: '-0.25rem', marginBottom: '0.5rem', minHeight: '3.5rem' }}>
                                Products which you promote in the cart, based on the current product.
                            </p>
                            <input
                                type="text"
                                className="form-control"
                            ...
                            {crossSells.length > 0 && (
                                <div className="product-tags-container">
                                    {crossSells.map((item, index) => (
                                        <span key={index} className="product-tag">
                                            {item}
                                            <button type="button" onClick={() => handleRemoveCrossSell(item)} className="product-tag-remove">&times;</button>
                                        </span>
                                    ))}
                                </div>
                            )}
                ...
                <div className="form-section">
                    ...
                    <div className="form-row">
                        <div className="form-group form-col" style={{ flex: 1 }}>
                            ...
                            <input type="number" className="form-control" placeholder="0.0" value={weight} onChange={(e) => setWeight(e.target.value)} />
                        </div>
                        <div className="form-group form-col" style={{ flex: 2 }}>
                            <label>Dimensions (cm)</label>
                            <div style={{ display: 'flex', gap: '0.5rem' }}>
                                <input type="number" className="form-control" placeholder="Length" value={length} onChange={(e) => setLength(e.target.value)} />
                                <input type="number" className="form-control" placeholder="Width" value={width} onChange={(e) => setWidth(e.target.value)} />
                                <input type="number" className="form-control" placeholder="Height" value={height} onChange={(e) => setHeight(e.target.value)} />
                            </div>
                        </div>
                        <div className="form-group form-col" style={{ flex: 1 }}>
                            <label>Shipping class</label>
                            <select className="form-control" value={shippingClass} onChange={(e) => setShippingClass(e.target.value)}>
                    ...
                    <div className="form-row" style={{ marginTop: '0.5rem' }}>
                        ...
                        <div className="form-group form-col">
                            ...
                            <select className="form-control" value={taxClass} onChange={(e) => setTaxClass(e.target.value)}>
                    ...
                    <div className="form-group" style={{ marginBottom: 0, marginTop: '0.5rem' }}>
                        ...
                        <p style={{ fontSize: '0.8rem', color: '#000', marginTop: '-0.25rem', marginBottom: '0.5rem' }}>Upload warranty, manuals, or specific policy documents (e.g. PDF, DOCX).</p>
                                ...
                                {policyDocuments.map((doc, index) => (
                                    ...
                                    >
                                        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', overflow: 'hidden', flex: 1 }}>
                                            <FileText size={18} color="#FF5722" style={{ flexShrink: 0 }} />
                                            <div style={{ display: 'flex', flexDirection: 'column', flex: 1, gap: '0.25rem' }}>
                                                <input
                                                    type="text"
                                                    value={doc.title}
                                                    onChange={(e) => handlePolicyTitleChange(index, e.target.value)}
                                                    placeholder="Document Title (e.g., Warranty Information)"
                                                    className="form-control"
                                                    style={{ height: '32px', fontSize: '0.85rem', maxWidth: '300px' }}
                                                />
                                                <span style={{ fontSize: '0.75rem', color: '#000', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                                                    File: {doc.name}
                                                </span>
                                            </div>
                                        </div>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', marginTop: '1rem' }}>
                    {submitError && (
                        <div style={{ color: '#FF5722', fontSize: '0.9rem', marginBottom: '1rem', fontWeight: 500 }}>
                            {submitError}
                        </div>
                    )}
                    <div className="form-footer-actions" style={{ marginTop: 0 }}>
                        <button className="btn-secondary" onClick={handleCancel}>Discard</button>
                        <button className="btn-primary" onClick={handleSaveProduct}>{isEditMode ? 'Update Product' : 'Save Product'}</button>
                    </div>
                </div >
